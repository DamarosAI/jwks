...
(function () {
  ...
  function ensureDialogStyles() {
    ...
    style.textContent = [
      ...
      "#" + DIALOG_ID + "{position:relative;width:min(100%,480px);max-height:min(92dvh,740px);overflow:auto;background:rgba(255,255,255,0.92);border:1px solid rgba(31,45,61,0.12);border-radius:16px;box-shadow:0 28px 70px rgba(16,22,29,0.2),inset 0 1px 0 rgba(255,255,255,0.85);padding:28px 28px 24px;box-sizing:border-box;animation:dm-pilot-in 280ms cubic-bezier(0.22,1,0.36,1);}",
      "@keyframes dm-pilot-fade{from{opacity:0;}to{opacity:1;}}",
      "@keyframes dm-pilot-in{from{opacity:0;transform:translateY(14px) scale(0.985);}to{opacity:1;transform:none;}}",
      "#" + DIALOG_ID + " .dm-pilot-head{display:flex;align-items:flex-start;gap:12px;margin:0 28px 18px 0;}",
      "#" + DIALOG_ID + " .dm-pilot-mark{flex:none;width:22px;height:24px;color:var(--ink-steel,#2f6193);margin-top:2px;}",
      "#" + DIALOG_ID + " .dm-pilot-eyebrow{margin:0 0 7px;font-family:var(--font-body,\"Hanken Grotesk\",system-ui,sans-serif);font-size:11px;font-weight:600;letter-spacing:0.18em;text-transform:uppercase;color:var(--ink-dim,#7b8794);}",
      "#" + DIALOG_ID + " .dm-pilot-title{margin:0 0 8px;font-family:var(--font-display,\"Archivo\",system-ui,sans-serif);font-size:clamp(1.35rem,3vw,1.65rem);font-weight:700;letter-spacing:-0.025em;line-height:1.15;color:var(--ink-cold,#10161d);}",
      "#" + DIALOG_ID + " .dm-pilot-lead{margin:0;font-family:var(--font-body,\"Hanken Grotesk\",system-ui,sans-serif);font-size:14.5px;line-height:1.55;color:var(--ink-quiet,#3c4955);}",
      "#" + DIALOG_ID + " .dm-pilot-lead strong{font-weight:600;color:var(--ink-steel,#2f6193);}",
      "#" + DIALOG_ID + " .dm-pilot-close{position:absolute;top:14px;right:14px;width:34px;height:34px;border:1px solid rgba(31,45,61,0.1);border-radius:999px;background:rgba(255,255,255,0.7);color:var(--ink-quiet,#3c4955);cursor:pointer;display:grid;place-items:center;}",
      "#" + DIALOG_ID + " .dm-pilot-close:hover{background:#fff;border-color:rgba(31,45,61,0.16);}",
      "#" + DIALOG_ID + " .dm-pilot-rule{height:1px;margin:0 0 18px;background:rgba(31,45,61,0.1);border:0;}",
      "#" + DIALOG_ID + " .dm-pilot-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px 12px;}",
      "#" + DIALOG_ID + " .dm-pilot-span{grid-column:1 / -1;}",
      "#" + DIALOG_ID + " label{display:flex;flex-direction:column;gap:6px;font-family:var(--font-mono,\"IBM Plex Mono\",ui-monospace,monospace);font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:var(--ink-dim,#7b8794);}",
      "#" + DIALOG_ID + " .dm-pilot-opt{font-weight:500;letter-spacing:0.04em;text-transform:none;color:var(--ink-faint,#586674);}",
      ...
      "#" + DIALOG_ID + " input::placeholder,#" + DIALOG_ID + " textarea::placeholder{color:var(--ink-dim,#7b8794);opacity:0.85;}",
      ...
      "#" + DIALOG_ID + " textarea{min-height:96px;resize:vertical;}",
      "#" + DIALOG_ID + " .dm-pilot-hp{position:absolute;left:-9999px;top:auto;width:1px;height:1px;overflow:hidden;}",
      "#" + DIALOG_ID + " .dm-pilot-actions{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:20px;}",
      ...
      "#" + DIALOG_ID + " .dm-pilot-cancel:hover{color:var(--ink-cold,#10161d);}",
      "#" + DIALOG_ID + " .dm-pilot-status{margin:12px 0 0;min-height:1.2em;font-family:var(--font-body,\"Hanken Grotesk\",system-ui,sans-serif);font-size:13px;line-height:1.4;color:var(--ink-quiet,#3c4955);}",
      "#" + DIALOG_ID + " .dm-pilot-status[data-tone=error]{color:#dc3a52;}",
      "#" + DIALOG_ID + " .dm-pilot-status[data-tone=ok]{color:#2e9e6b;}",
      "#" + DIALOG_ID + " .dm-pilot-route{margin-top:16px;padding-top:14px;border-top:1px solid rgba(31,45,61,0.1);font-family:var(--font-mono,\"IBM Plex Mono\",ui-monospace,monospace);font-size:10.5px;font-weight:500;letter-spacing:0.04em;color:var(--ink-dim,#7b8794);}",
      "#" + DIALOG_ID + " .dm-pilot-route b{font-weight:600;color:var(--ink-steel,#2f6193);}",
      "#" + DIALOG_ID + " .dm-pilot-success{display:none;text-align:center;padding:28px 8px 10px;}",
      "#" + DIALOG_ID + "[data-state=success] .dm-pilot-head,#" + DIALOG_ID + "[data-state=success] .dm-pilot-rule,#" + DIALOG_ID + "[data-state=success] form,#" + DIALOG_ID + "[data-state=success] .dm-pilot-route{display:none;}",
      "#" + DIALOG_ID + "[data-state=success] .dm-pilot-success{display:block;}",
      "#" + DIALOG_ID + " .dm-pilot-success-badge{width:48px;height:48px;margin:0 auto;border-radius:50%;display:grid;place-items:center;background:linear-gradient(180deg,#3f8f6d,#2e9e6b);box-shadow:inset 0 1px 0 rgba(255,255,255,0.22),0 10px 24px rgba(46,158,107,0.28);}",
      "#" + DIALOG_ID + " .dm-pilot-success h3{margin:16px 0 8px;font-family:var(--font-display,\"Archivo\",system-ui,sans-serif);font-size:22px;font-weight:700;letter-spacing:-0.02em;color:var(--ink-cold,#10161d);}",
      "#" + DIALOG_ID + " .dm-pilot-success p{margin:0;font-family:var(--font-body,\"Hanken Grotesk\",system-ui,sans-serif);font-size:14.5px;line-height:1.55;color:var(--ink-quiet,#3c4955);}",
      "@media (max-width:520px){#" + DIALOG_ID + " .dm-pilot-grid{grid-template-columns:1fr;}#" + DIALOG_ID + "{padding:24px 20px 20px;}}",
  ...
  function ensureDialog() {
    ...
    dialogRoot.innerHTML = [
      ...
      "    <h3>Inquiry sent</h3>",
      "    <p>Thanks — the team will follow up from team@damaros.ai.</p>",
      '    <div class="dm-pilot-actions" style="justify-content:center;margin-top:22px;">',
