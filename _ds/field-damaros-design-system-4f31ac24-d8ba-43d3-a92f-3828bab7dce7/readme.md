...
Positioning line (canonical, per damaros.ai, 2026-07-27): *the execution platform that makes sites trial-capable, and provable.* **AI-native where operations scale, deterministic where trust requires.** No model ever casts an eligibility verdict; PHI never enters an LLM path. (The prior hero, *the agentic execution platform for clinical trials*, is retired from the first screen per ADR-0002; see the note on "agentic" below.)
...
**On "agentic":** the word is approved for the operations layer and product copy, and it **no longer leads the hero** (ADR-0002). It was blessed when the reader was an academic center with an innovation budget; to a twelve-provider practice it is the word that closes the tab. The canonical hero is *the execution platform that makes sites trial-capable, and provable.* "Agentic" names the operations layer: Luna-governed agents stage, assemble, accelerate, and draft. It never names the verdict. The hard line holds regardless of the word: no model casts an eligibility verdict, PHI never enters a model, screening is deterministic. Use "agentic" for the platform; never say a model found a patient or decided eligibility.
